-- ============================================================================
-- VEHICLE AVAILABILITY SYSTEM ENHANCEMENT - FIXED VERSION
-- ============================================================================
...
-- 
-- FIXES:
-- 1. Fixed GIN index syntax error by separating index types
-- 2. Added proper column existence checks
-- 3. Optimized indexes for better performance
-- ============================================================================

-- Step 1: Enhance car_availability table structure with existence checks
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND table_name = 'car_availability' 
    AND column_name = 'availability_type'
  ) THEN
    ALTER TABLE car_availability 
    ADD COLUMN availability_type TEXT DEFAULT 'manual' 
    CHECK (availability_type IN ('manual', 'maintenance', 'personal', 'seasonal'));
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' 
    AND table_name = 'car_availability' 
    AND column_name = 'recurring_pattern'
  ) THEN
    ALTER TABLE car_availability ADD COLUMN recurring_pattern JSONB;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE car_availability ADD COLUMN created_by UUID REFERENCES user_profiles(id);
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE car_availability ADD COLUMN notes TEXT;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE car_availability ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
  END IF;
END $$;

-- Step 2: Create optimized performance indexes (FIXED)
-- Fix: Separate indexes for different column types and purposes

-- Index for car_availability table
...
-- Critical indexes for availability queries - optimized with WHERE clauses
...
-- FIXED: Separate optimized indexes for cars table instead of problematic composite GIN index
CREATE INDEX IF NOT EXISTS idx_cars_status_optimized 
ON cars(status) WHERE status = 'ACTIVE';

CREATE INDEX IF NOT EXISTS idx_cars_location_gin 
ON cars USING GIN(location);

CREATE INDEX IF NOT EXISTS idx_cars_search_composite 
ON cars(status, daily_rate) WHERE status = 'ACTIVE';

-- Additional performance indexes
CREATE INDEX IF NOT EXISTS idx_cars_daily_rate 
ON cars(daily_rate) WHERE status = 'ACTIVE';

-- Step 3: Add updated_at trigger for car_availability
-- Drop trigger if exists
DROP TRIGGER IF EXISTS update_car_availability_updated_at ON public.car_availability;

-- Create trigger
CREATE TRIGGER update_car_availability_updated_at 
...
BEGIN
  ...
  -- Check manual availability blocks (FIXED: Safe column reference)
  IF EXISTS (
    ...
    RETURN QUERY SELECT 
      ...
      (SELECT jsonb_agg(jsonb_build_object(
        ...
        'end_date', end_date, 
        'reason', reason,
        'availability_type', COALESCE(availability_type, 'manual'),
        'notes', notes
  ...
  WHILE v_current_date <= v_end_date LOOP
    ...
    IF EXISTS (
      ...
      RETURN QUERY SELECT 
        ...
        AND v_current_date BETWEEN start_date::DATE AND end_date::DATE
        LIMIT 1);
    -- Check for manual availability blocks (FIXED: Safe column reference)
    ELSIF EXISTS (
      SELECT 1 FROM car_availability 
      WHERE car_id = p_car_id 
      AND is_available = FALSE
      ...
      RETURN QUERY SELECT 
        ...
        (SELECT jsonb_build_object(
          'reason', reason,
          'type', COALESCE(availability_type, 'manual'),
          'notes', notes
...
-- View for host availability management dashboard (FIXED: Safe column reference)
...
SELECT 
  ...
  MAX(COALESCE(ca.updated_at, ca.created_at)) as last_availability_update
FROM cars c
...
DO $$
BEGIN
  RAISE NOTICE 'Vehicle Availability System installed successfully! (FIXED VERSION)';
  ...
  RAISE NOTICE 'FIXED: Separate optimized indexes created for better performance.';
  RAISE NOTICE 'FIXED: Column existence checks added for backward compatibility.';
END $$;